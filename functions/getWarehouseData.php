...
for ($i = 0; $i < count($result); $i++) {
    $type = $result[$i]['label'];
    $queryPrepared = $pdo->prepare("SELECT ingredientName, available FROM INGREDIENTS, STORE WHERE ingredientCategory = :type AND idIngredient = ingredient AND warehouse = :id");
